...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // Query current month aggregate only, then merge into store master list
        const { data: spcAggData, error: spcAggError } = await supabaseClient
            .from('agg_monthly_store')
            .select('store_id, agg_month, total_input, total_closed, total_pending, total_rejected, total_closing_direct, total_closing_followup')
        ...
        const result = (spcStores || []).map(store => {
            ...
            return {
                ...
                agg_month: aggRow?.agg_month || queryMonth,
